...
function speakText(text: string, onEnd: () => void) {
  ...
  const clean = removeEmojis(text); if (!clean) { onEnd(); return; }
...
function renderInlineCh(text: string, isUser: boolean): React.ReactNode[] {
  ...
  return text.replace(/<br\s*\/?>/gi,"\n").split(/(\*\*[\s\S]+?\*\*|\*[^*]+\*)/g).filter(Boolean).map((p,i) => {
    if (p.startsWith("**")&&p.endsWith("**")) return <strong key={i} className={`font-semibold ${hc}`}>{p.slice(2,-2)}</strong>;
    if (p.startsWith("*")&&p.endsWith("*")&&p.length>2) return <em key={i}>{p.slice(1,-1)}</em>;
    if (p.includes("\n")) return p.split("\n").map((s,si,a)=><span key={`${i}-${si}`}>{s}{si<a.length-1&&<br/>}</span>);
...
export default function ChatVoicePage() {
  ...
  const fetchChatSessions = useCallback(async () => {
    ...
    try { const r = await fetch("/api/chat/sessions?mode=chat"); if (r.ok) { const d = await r.json(); setChatSessions(d.sessions||[]); } }
    finally { setChatSessionsLoading(false); }
  }, []);

  const fetchVoiceSessions = useCallback(async () => {
    setVoiceSessionsLoading(true);
    try { const r = await fetch("/api/chat/sessions?mode=voice"); if (r.ok) { const d = await r.json(); setVoiceSessions(d.sessions||[]); } }
    finally { setVoiceSessionsLoading(false); }
  }, []);

  useEffect(() => { if (status === "authenticated") { fetchChatSessions(); fetchVoiceSessions(); } }, [status, fetchChatSessions, fetchVoiceSessions]);
  ...
  const handleVoiceSubmit = useCallback(async (text: string) => {
    ...
    try {
      const r = await fetch("/api/chat", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({message:msg,history:voiceMessages,mode:"voice"}) });
      const data = await r.json();
      if (!r.ok) { setVoiceError(data.message||"Failed"); setCallStatus("Error"); if (isInCall) { try { recognitionRef.current?.start(); } catch { /* ignore */ } } }
        ...
        if (sid) {
          fetch(`/api/chat/sessions/${sid}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({userMessage:msg,assistantMessage:data.message,mermaidCode:data.mermaidCode}) })
  ...
  }, [voiceMessages, isInCall, isListening, isMuted, voiceLoading]);
  ...
  const startCall = async () => {
    ...
    try {
      const r = await fetch("/api/chat/sessions", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({mode:"voice"}) });
      const data = await r.json();
  ...
  const deleteVoiceSession = async (id: string, e: React.MouseEvent) => {
    ...
    try { await fetch(`/api/chat/sessions/${id}`, { method:"DELETE" }); setVoiceSessions(p=>p.filter(s=>s._id!==id)); if (voiceSessionId===id) { setVoiceSessionId(null); setVoiceMessages([]); } }
    ...
    <main className="relative min-h-screen overflow-hidden flex flex-col bg-slate-100 dark:bg-[#0b1220]">
      ...
      <div className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-slate-200/50 dark:border-slate-700/50 bg-white/30 dark:bg-slate-900/30 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" onClick={() => setChatSidebarOpen(o => !o)} className="text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100">
            {chatSidebarOpen ? <PanelLeftClose className="w-5 h-5" /> : <PanelLeftOpen className="w-5 h-5" />}
          </Button>
        </div>
        <div className="flex items-center gap-2">
          <Zap className="w-5 h-5 text-blue-500" />
          <h1 className="hidden md:block text-lg font-semibold text-slate-800 dark:text-slate-200">AI Chat</h1>
          <button onClick={() => setActiveTab("voice")} title="Switch to Voice"
            className="ml-1 flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-violet-100 dark:hover:bg-violet-900/30 hover:text-violet-600 dark:hover:text-violet-400 hover:border-violet-300 dark:hover:border-violet-700 transition-all">
    ...
    <main className="h-screen flex flex-col bg-gray-900 overflow-hidden">
      <div className="relative flex-none flex items-center justify-between px-6 py-4 border-b border-gray-700 bg-gray-800/50 backdrop-blur-sm z-10">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" onClick={() => setVoiceSidebarOpen(o => !o)} className="text-gray-400 hover:text-blue-400 hover:bg-gray-700/60">
            {voiceSidebarOpen ? <PanelLeftClose className="w-5 h-5" /> : <PanelLeftOpen className="w-5 h-5" />}
          </Button>
        </div>
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
          <h1 className="hidden md:block text-lg font-semibold text-white">Voice AI</h1>
